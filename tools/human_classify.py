...
class DrawBBox(tk.Tk):
    def __init__(self):
        ...
        # IMPORTANT: must be consistent with rest of the project
        # self.CLASSES = [
        #     ('Tartaruga cabeçuda', 0),
        #     ('Tartaruga de couro', 1),
        #     ('Tartaruga oliva', 2),
        #     ('Tartaruga de pente', 3),
        #     ('Tartaruga verde', 4)
        # ]
        self.CLASSES = [
            ('Impossivel', 0),
        ...
        self.human_class = tk.IntVar()
        self.human_class.set(-1)
        self.radio_buttons = []
        for text, mode in self.CLASSES:
            b = tk.Radiobutton(self.classFrame, text=text,
                            variable=self.human_class, value=mode, indicatoron=0)
            b.pack(anchor=tk.W, pady=10, fill=tk.X, ipady=15)
    ...
    def __clean_state(self):
        ...
        [b.deselect() for b in self.radio_buttons]
        self.human_class.set(-1)
        self.num_image.set('%d / %d' % (self.current_index, len(self.images)))
    ...
    def on_ok(self):
        try:
            class_ = self.human_class.get()
            if class_ not in [c[1] for c in self.CLASSES]:
                raise ValueError('Escolha a especie antes')

            # Store annotations
            (x0, y0, x1, y1) = self.current_bbox
            if x1 - x0 < 10 or y1 - y0 < 10:
                raise ValueError('Desenhe uma bounding box valida')
            line = '%s %d %d %d %d %d\n' % (self.photo.name, self.human_class.get(), x0, y0, x1, y1)
